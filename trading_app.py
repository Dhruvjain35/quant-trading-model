...
Tax Model (BlackRock / Schwab methodology):
    - Lot-level holding-period tracking (ST < 252 days, LT >= 252 days)
    - Automated tax-loss harvesting (TLH) triggered at > 5% unrealized loss
    - Gain-loss matching: ST losses offset ST gains first, then LT gains
    - After-tax objective function: maximize E[r_net] not E[r_gross]
    - Low-turnover conviction threshold (0.55) defers ST gains to LT
    - ETF vehicle: tax-efficient, no internal capital gains distributions
    - Loss carry-forward tracked and reported
"""
...
html, body, [class*="css"] { font-family: 'Rajdhani', sans-serif; background-color: #050d1a; color: #c8d8e8; }
.stApp { background-color: #050d1a; }
[data-testid="stSidebar"] { background-color: #07111f; border-right: 1px solid #0d2137; }
...
.main-title { font-family: 'Rajdhani', sans-serif; font-size: 2.6rem; font-weight: 700; background: linear-gradient(90deg,#00ffe0,#00c8ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; letter-spacing: 2px; margin-bottom: 0; }
.subtitle-bar { font-family: 'Share Tech Mono', monospace; font-size: 0.65rem; color: #2a7a9a; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 1.2rem; }
.section-header { font-family: 'Share Tech Mono', monospace; font-size: 0.75rem; color: #00ffe0; letter-spacing: 3px; text-transform: uppercase; border-bottom: 1px solid #0d2a3d; padding-bottom: 6px; margin: 1.6rem 0 1rem 0; }
.metric-card { background: #07111f; border: 1px solid #0d2a3d; border-top: 2px solid #00ffe0; border-radius: 4px; padding: 14px 18px; margin: 4px 0; }
...
.metric-label { font-family: 'Share Tech Mono', monospace; font-size: 0.6rem; color: #3a6a8a; letter-spacing: 2px; text-transform: uppercase; }
.bench-label { font-family: 'Share Tech Mono', monospace; font-size: 0.62rem; color: #2a5a7a; margin-top: 2px; }
.hyp-box { background: #07111f; border: 1px solid #0d2a3d; border-left: 3px solid #00ffe0; border-radius: 4px; padding: 16px 20px; font-family: 'Share Tech Mono', monospace; font-size: 0.7rem; color: #7ab8d4; line-height: 1.8; margin-bottom: 1rem; }
.hyp-title { font-size: 0.6rem; color: #00ffe0; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 8px; }
.banner-green { background: #041a0e; border: 1px solid #00804a; border-radius: 3px; padding: 8px 16px; font-family: 'Share Tech Mono', monospace; font-size: 0.68rem; color: #00c87a; }
.banner-yellow { background: #1a1400; border: 1px solid #806a00; border-radius: 3px; padding: 8px 16px; font-family: 'Share Tech Mono', monospace; font-size: 0.68rem; color: #f4c542; }
.crisis-table { width: 100%; border-collapse: collapse; font-family: 'Share Tech Mono', monospace; font-size: 0.7rem; }
.crisis-table th { color: #2a6a8a; letter-spacing: 2px; text-transform: uppercase; padding: 8px 12px; text-align: right; border-bottom: 1px solid #0d2a3d; font-size: 0.6rem; }
...
.badge-green { background: #041a0e; border: 1px solid #00804a; border-radius: 3px; padding: 2px 8px; color: #00c87a; font-size: 0.65rem; }
.stButton > button { background: linear-gradient(135deg,#00ffe0,#00a8c8) !important; color: #050d1a !important; font-family: 'Rajdhani', sans-serif !important; font-weight: 700 !important; font-size: 0.9rem !important; letter-spacing: 2px !important; border: none !important; border-radius: 4px !important; padding: 12px 20px !important; width: 100% !important; text-transform: uppercase !important; }
.stTextInput input { background: #0a1825 !important; border: 1px solid #0d2a3d !important; color: #c8d8e8 !important; font-family: 'Share Tech Mono', monospace !important; border-radius: 3px !important; }
div[data-testid="stNumberInput"] input { background: #0a1825 !important; border: 1px solid #0d2a3d !important; color: #c8d8e8 !important; font-family: 'Share Tech Mono', monospace !important; }
...
def plot_layout(**overrides):
    """
    Returns a fresh layout dict for Plotly.
    Uses explicit keyword merging — never unpacks a dict that contains
    a key that is also in overrides (fixes the yaxis TypeError).
    """
    base = dict(
    ...
    # Deep-merge overrides so callers can replace yaxis cleanly
...
# ─────────────────────────────────────────────
# FEATURE ENGINEERING  (all features lagged >= 1 day — no leakage)
# ─────────────────────────────────────────────
...
def build_features(prices, returns, vix, risky_col, safe_col):
...
# ─────────────────────────────────────────────
# Tax rates
ST_RATE   = 0.35   # short-term gains (<252 trading days)
LT_RATE   = 0.20   # long-term gains  (>=252 trading days)
TLH_THRESHOLD = -0.05   # harvest unrealized loss if < -5%
SLIPPAGE_BPS  = 5
COMMISSION_BPS = 3
ROUND_TRIP = (SLIPPAGE_BPS + COMMISSION_BPS) / 10_000


def run_tax_aware_backtest(signal_series, r_risky, r_safe):
    """
    Simulate after-tax returns with:
      1. Lot-level holding-period tracking
      2. Automated tax-loss harvesting at -5% unrealized threshold
      3. ST/LT gain-loss matching
      4. Transaction costs only on signal flips
      5. Deferred taxation on unrealized gains (only taxed at realization)
    """
    dates       = signal_series.index
    n           = len(dates)
    nav         = 1.0
    lot_basis   = None       # (cost_basis_nav, entry_day_index)
    current_pos = -1         # -1 = not yet initialized
    st_loss_bank = 0.0
    lt_loss_bank = 0.0

    net_r_arr = np.zeros(n)
    nav_arr   = np.zeros(n)

    for i in range(n):
        sig   = int(signal_series.iloc[i])
        r_day = float(r_risky.iloc[i]) if sig == 1 else float(r_safe.iloc[i])

        need_trade = sig != current_pos

        if need_trade and current_pos != -1 and lot_basis is not None:
            holding_days = i - lot_basis[1]
            unrealized   = nav / lot_basis[0] - 1.0

            # Trigger TLH proactively if unrealized < threshold
            if unrealized < TLH_THRESHOLD or need_trade:
                gain    = nav - lot_basis[0]
                is_lt   = holding_days >= 252

                if gain < 0:
                    # Realized loss — bank by holding type
                    if is_lt:
                        lt_loss_bank += abs(gain)
                    else:
                        st_loss_bank += abs(gain)
                    tax_owed = 0.0
                else:
                    # Realized gain — offset banked losses (matching type first)
                    if is_lt:
                        # LT losses offset LT gains first
                        lt_offset = min(lt_loss_bank, gain)
                        lt_loss_bank -= lt_offset
                        remainder = gain - lt_offset
                        # Then ST losses can offset remaining LT gain (conservative)
                        st_offset = min(st_loss_bank, remainder)
                        st_loss_bank -= st_offset
                        net_gain = remainder - st_offset
                        tax_owed = net_gain * LT_RATE
                    else:
                        # ST losses offset ST gains first
                        st_offset = min(st_loss_bank, gain)
                        st_loss_bank -= st_offset
                        net_gain = gain - st_offset
                        tax_owed = net_gain * ST_RATE

                nav -= tax_owed
                nav -= ROUND_TRIP * nav   # transaction cost on flip
                lot_basis = (nav, i)
        ...
        nav_before    = nav
        nav          *= (1.0 + r_day)
        net_r_arr[i]  = nav / nav_before - 1.0
        nav_arr[i]    = nav

    loss_carryforward = st_loss_bank + lt_loss_bank
    return (
        pd.Series(net_r_arr, index=dates, name="strategy"),
        pd.Series(nav_arr,   index=dates, name="nav"),
        loss_carryforward,
    )
...
def _ann_return(r):
    ...
    if len(r) < 2:
        return 0.0
    return float((1 + r).prod() ** (1 / (len(r) / 252)) - 1)
...
def run_pipeline(df, returns, risky_col, safe_col, embargo_months=4, n_mc=500):
    ...
    X, y, idx = df[fc].values, df["target"].values, df.index
    ...
    gb.fit(X_tr_s, y_tr); rf.fit(X_tr_s, y_tr); lr.fit(X_tr_s, y_tr)
    ...
    # 0.55 threshold — low-turnover bias to defer ST gains to LT
    signal    = (p_ens > 0.55).astype(int)
    sig_s     = pd.Series(signal, index=idx_te)
    r_risky   = returns[risky_col].reindex(idx_te)
    r_safe    = returns[safe_col].reindex(idx_te)
    bench_r   = r_risky.copy().rename("benchmark")

    strat_r, nav_series, loss_cf = run_tax_aware_backtest(sig_s, r_risky, r_safe)
    ...
    strat_is, _, _ = run_tax_aware_backtest(sig_is, r_r_is, r_s_is)
    ...
    shap_vals = expl.shap_values(X_te_s[:500])
    ...
    for _ in range(1000):
        ...
        pr = np.where(ps == 1, r_risky.values, r_safe.values)
        perm_sh.append(_sharpe(pd.Series(pr, index=idx_te)))
    ...
    nd       = len(strat_r)
    mc_paths = np.array([np.cumprod(1 + rng2.choice(strat_r.values, nd, replace=True))
                         for _ in range(n_mc)])
    ...
    # TC sensitivity (pre-tax gross, for stress-test only)
    trades_mask = sig_s.diff().abs().fillna(0)
    tc_results  = []
    r_gross     = np.where(signal == 1, r_risky.values, r_safe.values)
    for bps in [0, 5, 10, 20, 30, 50]:
        tc_r = pd.Series(r_gross - trades_mask.values * bps / 10_000, index=idx_te)
    ...
    return dict(
        ...
        equity_strat=(1+strat_r).cumprod(), equity_bench=(1+bench_r).cumprod(),
        drawdown_strat=_rolling_drawdown(strat_r), drawdown_bench=_rolling_drawdown(bench_r),
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown('<p style="font-family:Share Tech Mono;font-size:0.6rem;letter-spacing:3px;color:#3a8aaa;text-transform:uppercase;">MODEL CONTROLS</p>', unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown('<p style="font-family:Share Tech Mono;font-size:0.58rem;color:#2a5a7a;line-height:1.8;">Regime-Filtered Boosting · Purged walk-<br>forward validation · Ensemble voting · SHAP<br>attribution · Permutation testing</p>', unsafe_allow_html=True)
...
<span style="color:#1a5a7a;">Test: Signal permutation (n=1,000+) &nbsp;|&nbsp; Threshold: p &le; 0.05 &nbsp;|&nbsp; Alpha: OLS GLM on excess returns</span>
</div>
""", unsafe_allow_html=True)
st.markdown("---")
...
if run_btn or "results" in st.session_state:
    ...
    if run_btn:
        with st.spinner("Running research pipeline..."):
            try:
                ...
                res = run_pipeline(df_feat, returns, risky_asset, safe_asset, embargo, int(n_mc))
    ...
    for dt, v in sig_s.items():
        ...
        elif v == 0 and in_r:     in_r = False;  fig_eq.add_vrect(x0=rstart, x1=dt, fillcolor="rgba(0,255,224,0.04)", line_width=0, row=1, col=1)
    if in_r: fig_eq.add_vrect(x0=rstart, x1=sig_s.index[-1], fillcolor="rgba(0,255,224,0.04)", line_width=0, row=1, col=1)

    fig_eq.add_trace(go.Scatter(x=eq_b.index, y=eq_b.values, name=f"{risky_col} Buy & Hold", line=dict(color="rgba(200,200,200,0.4)", dash="dash", width=1.5)), row=1, col=1)
    fig_eq.add_trace(go.Scatter(x=eq_s.index, y=eq_s.values, name="AMCE Strategy", line=dict(color=CYAN, width=2.5)), row=1, col=1)
    fig_eq.add_trace(go.Scatter(x=res["drawdown_strat"].index, y=res["drawdown_strat"].values, name="Strategy DD", fill="tozeroy", fillcolor="rgba(255,77,109,0.25)", line=dict(color=PINK, width=1)), row=2, col=1)
    fig_eq.add_trace(go.Scatter(x=res["drawdown_bench"].index, y=res["drawdown_bench"].values, name="Bench DD", fill="tozeroy", fillcolor="rgba(100,100,120,0.15)", line=dict(color="rgba(180,180,200,0.4)", width=1)), row=2, col=1)
    ...
    p5 = np.percentile(mc, 5, axis=0); p95mc = np.percentile(mc, 95, axis=0); med = np.median(mc, axis=0)
    fig_mc = go.Figure()
    fig_mc.add_trace(go.Scatter(x=np.concatenate([xd,xd[::-1]]), y=np.concatenate([p95mc,p5[::-1]]), fill="toself", fillcolor="rgba(26,42,80,0.7)", line=dict(width=0), name="95% Confidence Cone"))
    fig_mc.add_trace(go.Scatter(x=xd, y=med, line=dict(color="rgba(0,200,180,0.6)", dash="dash", width=1.5), name="Median Expectation"))
    fig_mc.add_trace(go.Scatter(x=np.arange(len(eq_s)), y=eq_s.values, line=dict(color=CYAN, width=2.5), name="Actual Strategy"))
    fig_mc.add_trace(go.Scatter(x=np.arange(len(eq_b)), y=eq_b.values, line=dict(color="rgba(200,200,220,0.4)", width=1.5, dash="dot"), name=f"{risky_col} Buy & Hold"))
    ...
    with cl:
        ...
        fig_rs.add_trace(go.Scatter(x=res["roll_sh_b"].dropna().index, y=res["roll_sh_b"].dropna().values, line=dict(color="rgba(200,200,220,0.4)", width=1, dash="dot"), name=f"{risky_col} B&H"))
        fig_rs.add_trace(go.Scatter(x=res["roll_sh_s"].dropna().index, y=res["roll_sh_s"].dropna().values, line=dict(color=CYAN, width=2), fill="tozeroy", fillcolor="rgba(0,255,224,0.06)", name="Strategy"))
    ...
    with cr:
        fig_rw = go.Figure()
        fig_rw.add_trace(go.Scatter(x=res["roll_wr"].dropna().index, y=res["roll_wr"].dropna().values, line=dict(color=CYAN, width=2), fill="tozeroy", fillcolor="rgba(0,255,224,0.06)", name="Win Rate"))
        fig_rw.add_hline(y=0.5, line_color=PINK, line_dash="dash", line_width=1)
        # KEY FIX: pass yaxis directly to plot_layout so no duplicate key conflict
    ...
    with c2:
        ...
        st_tag = '<span style="color:#00c87a;font-size:0.65rem;">alpha-significant</span>' if res["perm_p_value"]<0.05 else ""
    ...
    fig_p = go.Figure()
    fig_p.add_trace(go.Histogram(x=res["perm_sharpes"], nbinsx=50, marker_color="rgba(60,80,120,0.8)", name="Random Signal Distribution"))
    fig_p.add_vline(x=res["pct_95"], line_color=PINK, line_dash="dash", line_width=1.5, annotation_text=f"95th Perm. Pct = {res['pct_95']:.3f}", annotation_font_color=PINK, annotation_font_size=10)
    fig_p.add_vline(x=sh_s, line_color=CYAN, line_width=2, annotation_text=f"Actual Sharpe = {sh_s:.4f}", annotation_font_color=CYAN, annotation_font_size=10)
    ...
    st.markdown(f'<p style="font-family:Share Tech Mono;font-size:0.62rem;color:#2a5a7a;">Institutional viability requires positive risk-adjusted returns after realistic trading frictions.</p>', unsafe_allow_html=True)
    st.markdown(f'<p style="font-family:Share Tech Mono;font-size:0.65rem;color:#3a7a9a;">Estimated trades: {res["n_trades"]} &nbsp;|&nbsp; Approx. annual turnover: {ann_to:.0f}x</p>', unsafe_allow_html=True)
    ...
    st.markdown('<p style="font-family:Share Tech Mono;font-size:0.62rem;color:#2a5a7a;">Convergence = high conviction. Divergence = regime ambiguity.</p>', unsafe_allow_html=True)
    fig_en = go.Figure()
    ...
    fig_en.add_hline(y=0.5, line_color="rgba(255,255,255,0.3)", line_dash="dash", line_width=1, annotation_text="Neutral Threshold (0.5)", annotation_font_size=9)
    fig_en.update_layout(**plot_layout(height=350, yaxis=dict(gridcolor=GRID_COLOR, zerolinecolor=GRID_COLOR, color=WHITE, range=[0,1], title="Risky Asset Positive")))
    ...
    st.markdown('<p style="font-family:Share Tech Mono;font-size:0.62rem;color:#2a5a7a;">SHapley Additive eXplanations decompose each prediction into feature contributions using cooperative game theory. Red = pushes toward risky asset. Blue = toward safe asset.</p>', unsafe_allow_html=True)
    ...
    with cs2:
        ...
        for feat,fi in zip(tf,ti):
            ...
            fig_bee.add_trace(go.Scatter(x=s2+np.random.normal(0,0.002,len(s2)), y=[feat]*len(s2),
                mode="markers", marker=dict(size=2.5,color=colors,opacity=0.7), name=feat, showlegend=False))
    ...
    font-size:0.68rem;color:#7ab8d4;line-height:2.0;">
    <div style="font-size:0.6rem;color:#f4c542;letter-spacing:4px;text-transform:uppercase;
    ...
    return of {ann_s*100:.1f}% (vs. benchmark {ann_b*100:.1f}%) on an after-tax, after-friction basis.
    OLS alpha decomposition yields {res['ols_alpha']*100:+.2f}% annualized excess return
    (&beta;={res['ols_beta']:.2f}). Permutation testing across 1,000 signal shuffles
    {'rejects' if h1 else 'fails to reject'} H0 at p={res['perm_p_value']:.4f}.
    Purged walk-forward validation with {embargo}-month embargo eliminates look-ahead bias.
    Tax model: lot-level holding-period tracking, automated TLH at &gt;5% unrealized loss,
    ST/LT gain-loss matching, low-turnover threshold (0.55 ensemble conviction) defers
    short-term gains to long-term where feasible. Remaining loss carry-forward: ${lc:.4f}.
    Bootstrap Monte Carlo ({int(n_mc)} simulations) confirms {pb*100:.0f}% probability of
    benchmark outperformance. Ensemble decay: {ds:.0f}% Sharpe ratio decay training-to-test.
    </div>
    """, unsafe_allow_html=True)
