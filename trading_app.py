...
/* Buttons */
.stButton button {
    background: linear-gradient(135deg, #00FFB2, #00D99A) !important;
...
def run_ensemble_backtest(X, y, vix_threshold=25, min_hold_days=30):
    ...
    for i in range(train_window, len(X), 21):
        ...
        if not in_crisis:
            ...
            if votes_bullish >= 3: signal = 1
            elif votes_bearish >= 3: signal = -1
            else: signal = 0
...
def permutation_test(signals, returns, risky_ticker, safe_ticker, n_perms=1000):
    ...
    for _ in range(n_perms):
        ...
        if s > 0: perm_sharpes.append((m/s) * np.sqrt(252))
...
# ============================================================
# 3D MONTE CARLO FUNCTION (ADDED TO FIX MISSING FUNCTION ERROR)
# ============================================================
def create_3d_monte_carlo(results_df, n_sims=200):
    ...
    for _ in range(n_sims):
        ...
        mc_paths.append(np.cumprod(1 + boot_rets))
        
    z_data = np.array(mc_paths)
    ...
    fig = go.Figure(data=[go.Surface(
        ...
        contours={"z": {"show": True, "start": 0.5, "end": 2.0, "size": 0.1}}
    )])
    
    fig.update_layout(
        title='3D Wealth Surface (Bootstrap Resampling)',
        scene=dict(
...
if not run_analysis:
    ...
    perf_df = pd.DataFrame({
        ...
        "Strategy (NET)": [f"{sh_net:.3f}", f"{tot_net*100:.0f}%", f"{((1+tot_net)**(252/len(results_df))-1)*100:.1f}%", f"{dd_net*100:.1f}%", f"{(results_df['net_ret']>0).mean():.1%}"],
        "Benchmark": [f"{sh_bench:.3f}", f"{tot_bench*100:.0f}%", f"{((1+tot_bench)**(252/len(results_df))-1)*100:.1f}%", f"{dd_bench*100:.1f}%", f"{(results_df['bench_ret']>0).mean():.1%}"]
    })
    ...
    fig_eq.add_trace(go.Scatter(x=cum_bench.index, y=cum_bench.values, name=f'{risky_ticker} Buy & Hold', line=dict(color='#8B95A8', width=2, dash='dot'), opacity=0.7))
    fig_eq.add_trace(go.Scatter(x=cum_net.index, y=cum_net.values, name='MRAEM Strategy (NET)', line=dict(color='#00FFB2', width=3), fill='tonexty', fillcolor='rgba(0, 255, 178, 0.05)'))
    
    crisis_dates = results_df[results_df['in_crisis']].index
    for cd in crisis_dates[::20]: 
        fig_eq.add_vrect(x0=cd, x1=cd, fillcolor="rgba(255, 159, 67, 0.1)", layer="below", line_width=0)
    
    fig_eq.update_layout(title='Cumulative Returns: Strategy vs Benchmark', xaxis_title='Date', yaxis_title='Portfolio Value (×)', hovermode='x unified', height=500, paper_bgcolor='#0A0E1A', plot_bgcolor='#0F1419', font=dict(family='IBM Plex Sans', color='#E8EFF7'), legend=dict(bgcolor='#141921', bordercolor='#1A1F2E'))
    st.plotly_chart(fig_eq, use_container_width=True)
    
    # ============================================================
    # 2D MONTE CARLO - PROPERLY PLACED HERE
    # ============================================================
    ...
    fig_mc.add_trace(go.Scatter(x=list(range(len(percentile_95))) + list(range(len(percentile_5)))[::-1], y=list(percentile_95) + list(percentile_5)[::-1], fill='toself', fillcolor='rgba(0, 255, 178, 0.12)', line=dict(color='rgba(0, 255, 178, 0)'), name='95% Confidence Interval'))
    fig_mc.add_trace(go.Scatter(x=list(range(len(percentile_50))), y=percentile_50, line=dict(color='#5A6170', width=2, dash='dot'), name='Median Expectation'))
    fig_mc.add_trace(go.Scatter(x=list(range(len(cum_net))), y=cum_net.values, line=dict(color='#00FFB2', width=3), name='Actual Strategy'))

    fig_mc.update_layout(title='2D Monte Carlo Bootstrap Resampling (500 paths)', xaxis_title='Trading Days', yaxis_title='Portfolio Value (×)', height=450, paper_bgcolor='#0B0E14', plot_bgcolor='#11141C', font=dict(family='Inter', color='#EBEEF5', size=11))
    ...
    # 3D MONTE CARLO
    # ============================================================
    st.markdown("## 04 — 3D Monte Carlo Wealth Surface")
    ...
    # ============================================================
    st.markdown("## 05 — Statistical Validation Panel")
    col_stat1, col_stat2 = st.columns(2)
    
    with col_stat1:
        st.markdown("### Permutation Test Results")
        fig_perm = go.Figure()
        fig_perm.add_trace(go.Histogram(x=perm_sharpes, nbinsx=50, name='Random Strategies', marker=dict(color='#8B95A8', opacity=0.6)))
        fig_perm.add_vline(x=actual_sh, line=dict(color='#00FFB2', width=3), annotation_text=f"Actual: {actual_sh:.3f}", annotation_position="top")
        fig_perm.add_vline(x=np.percentile(perm_sharpes, 95), line=dict(color='#FF9F43', width=2, dash='dash'), annotation_text="95th %ile", annotation_position="top")
        
        fig_perm.update_layout(title=f'Permutation Test (p={p_val:.4f})', xaxis_title='Sharpe Ratio', yaxis_title='Frequency', height=400, paper_bgcolor='#0A0E1A', plot_bgcolor='#0F1419', font=dict(family='IBM Plex Sans', color='#E8EFF7'))
    ...
    with col_stat2:
        ...
        # Finished your truncated code here
        fig_boot.add_trace(go.Histogram(
            x=boot_sharpes, 
            nbinsx=50, 
            name='Bootstrap Distribution', 
            marker=dict(color='#00FFB2', opacity=0.7)
        ))
        
        fig_boot.add_vline(x=sh_lower, line=dict(color='#FF9F43', width=2, dash='dash'), annotation_text="Lower CI")
        fig_boot.add_vline(x=sh_upper, line=dict(color='#FF9F43', width=2, dash='dash'), annotation_text="Upper CI")
        
        fig_boot.update_layout(
            title=f'95% Confidence Interval: [{sh_lower:.2f}, {sh_upper:.2f}]', 
